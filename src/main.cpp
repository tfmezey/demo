	...
	while( (option = getopt_long(argc, argv, "bt:h", long_options, nullptr)) != -1 )
	{
		switch(option)
		{
			...
			case 't':
			{
				...
				if(result == algorithms::readNumber::token::NATURAL)
...
int main(int argc, char *argv[])
{
	...
	long rk4Trials = 0;
	if(ProcessArgs(argc, argv, rk4Trials) == false)
		return -1;
